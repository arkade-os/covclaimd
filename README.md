...
```sh
make test            # unit tests (excludes test/e2e)
...
The e2e suite drives `covclaimd` in-process against a real arkd + emulator
stack on [nigiri](https://github.com/vulpemventures/nigiri) regtest.

```sh
make setup-test-env    # start nigiri + the arkd/emulator stack (test/docker-compose.yml)
make integrationtest   # go test ./test/e2e/...
make teardown-test-env # stop everything
```

The stack maps arkd to `localhost:7170/7171` and the emulator to
`localhost:7173` (see `test/docker-compose.yml`).

> **Note on flaky DNS:** `make integrationtest` dials `localhost` services over
> gRPC, whose default resolver issues a `_grpc_config.<host>` TXT lookup per
> channel. On networks with an unreliable DNS server (e.g. a phone hotspot)
> those lookups can stall and trip the readiness timeout. Run the tests on a
> connection with a dependable resolver if you hit `failed waiting for
> covclaimd readiness: context deadline exceeded`.

## CI

GitHub Actions run on PRs:

- `.github/workflows/unit.yaml` — linting, `go vet`, unit tests (`make test`), security check.
- `.github/workflows/integration.yaml` — spins up nigiri + the arkd/emulator stack and runs `make integrationtest`.
