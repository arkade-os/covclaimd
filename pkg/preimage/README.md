# `pkg/preimage` — Preimage-claim covclaimd plugin

A `covclaimd.Plugin` that claims preimage-gated VTXOs the moment their
funding tx is broadcast. The maker encrypts a 32-byte preimage to the
covclaimd's ECIES key and stamps it (plus the receiver script) into the
...
1. It carries an ark OP_RETURN extension (parsed via `extension.NewExtensionFromTx`).
2. The extension contains a `ClaimPacket` (`PacketType = 0x04`) via `FindClaim`.
3. `Decrypt(covclaimdPrivKey, packet.Ciphertext)` succeeds and yields a 32-byte preimage.
...
Failures at any step are silent skips (`decode` returns `ok == false`, or the output loop `continue`s).

Produced intent: `MatchedClaim{Outpoint, Amount, Credentials{Preimage, ArkadeScript, Taptree, PkScript}}`.

## Spendability gate

After `decode` succeeds, `Match` runs one I/O gate before accepting the intent:

| Gate | Source | Reject when |
|---|---|---|
| `vtxoSpendable` | `arkClient.Indexer().GetVtxos(WithScripts, WithSpendableOnly)` | The funding VTXO isn't yet (or no longer) spendable for our script |

Single gate by design — decode already encodes most of the eligibility.
...
plugin, err := preimage.NewPlugin(ctx, preimage.Config{
    ...
    CovclaimdPrivKey:       covclaimdPriv,          // ECIES privkey; derived from wallet seed via HMAC-SHA256
...
exposes `CovclaimdPubKey()` / `EmulatorPubKey()` so makers can encrypt preimages
against the right keys. `cmd/covclaimd` fetches ark/emulator metadata, constructs
the plugin, and wires it into the shared covclaimd runtime.

## Files quick-reference

- `plugin.go` — `NewPlugin` plus the `covclaimd.Plugin` methods implemented directly: `Filter`/`Match` (`decode` + `vtxoSpendable`)/`Solve` (`claim`).
...
- `maker.go` — `BuildPacket` helper used by funders to encrypt a preimage to the covclaimd and produce the matching `extension.Packet`.
